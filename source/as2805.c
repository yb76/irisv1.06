...
void AS2805BufferUnpack(char * data, uchar format, uint size, uchar * buffer, uint * index)
{
	...
	switch (format)
	{
		...
		case C_hhmmss:
			...
			break;
		case C_BITMAP:
			UtilHexToString(&buffer[*index], size/8, data);
			*index += size/8;
			break;
		case C_STRING:
		...
		case C_LLLVAR:
			...
			UtilHexToString(&buffer[*index], size, data);
			*index += size;
			break;
		default:
			break;
...
void AS2805Unpack(uchar field, char * data, uchar * buffer, uint length)
{
	...
	for (fieldsIndex = 10; currentField <= (secondary?128:64) && fieldsIndex < length; currentField++, bitMap >>= 1)
	{
		if (bitMap == 0)
		{
			bitMap = 0x80;
